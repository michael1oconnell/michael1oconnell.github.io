...
if (importBtn) {
	...
	jQuery('.wp-jelly-btn-yes').on('click', function(e) {
		...
		if (base64regex.test(codetext)===false) {
			wpjellySetErrorMessage("Not a valid key");
			return false;
		}

		var mainurl = 'https://wpjelly.com/wp-admin/admin-ajax.php';

		if ( typeof window.atob === "function" ) {
			var codevalues = window.atob( codetext );

			if ( codevalues ) {
				var codeparts = codevalues.split( '|' );

				if ( codeparts ) {
					if ( ( typeof codeparts['2'] !== 'undefined' ) && ( codeparts['2'] === 'dev' ) ) {
	...
	function wpjellyReportError(content) {
        $url = 'https://wpjelly.com/wp-json/audit/v1/report';

		jQuery.post( $url, {
			'site_url': wpjellyControl.site_url,
			'content': content
		} );
	}

	function wpjellySetErrorMessage(message) {
		jQuery(".wp-jelly-modal").fadeOut();
	...
	function wpjellyErrorOccured(message) {
		if ( confirm( 'Failed to import your website. Would you like to send debug report to WP Jelly team?' ) ) {
			var content = "Message: " + message + "\n" + "Progress: " + jQuery(".wpjelly-import-percentage").text();
			wpjellyReportError(content);
		}

		wpjellySetErrorMessage(message);
	}
	...
	function wpjellyFailedToProcessRequest(jqXHR, textStatus, errorThrown) {
		...
		if ( confirm( 'Failed to import your website. Would you like to send debug report to WP Jelly team?' ) ) {
			...
			if ( textStatus ) {
				content += "Text Status: " + textStatus + "\n";
			}

			if ( errorThrown ) {
				content += "Error Thrown: " + errorThrown + "\n";
			}

			content += "Progress: " + jQuery(".wpjelly-import-percentage").text();

			wpjellyReportError(content);
		}

		wpjellySetErrorMessage("Failed to process this request");
	}
